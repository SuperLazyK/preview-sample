...
class StickJointLink(JointLink):
    def __init__(self, m, l, joint, cx = None, Icog=None, XT=None, q=0, dq=0):
        if Icog is None:
            Icog = stickI(m,l)
        if cx is None:
            I = mcI(m, [l/2, 0], Icog)
...
class StickSpringJointLink(StickJointLink):
    def __init__(self, m, l, k, joint, q=0, dq=0):
        super().__init__(m, l, joint, q=q, dq=dq)
        self.k = k
...
class WheelJointLink(JointLink):
    def __init__(self, m, r, joint, XT=None, q=0, dq=0):
        I = mcI(m, [0, 0], circleI(m,r))
        if XT is None:
            XT = Xpln(0, [0, 0])
        super().__init__(m, I, XT, joint)
        self.Ic = Ic(m, circleI(m,r))
        self.geometries = [GeoCircle(r), GeoLineSegment(r)]
        self.Xc = Xpln(0, [0, 0])
