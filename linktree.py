...
class LinkTreeModel:
    ...
    def __init__(self, jointlinks, parent_idx=None, g_acc=9.8):
        ...
        self.X_parent_to = np.zeros((self.NB, self.dim, self.dim))
        self.vel = np.zeros((self.NB, self.dim)) # vel of body i in body i coordinate
    ...
    def composite_inertia(self):
        ...
        for i in range(NB-1, -1, -1):
            ...
            if j != -1: # parent is root
                ...
                Ic[j] = Ic[j] + X_j_to_i.T @ self.jointlinks[i].I @ X_j_to_i
            F = Ic[i] @ S[i]
            H[i][i] = S[i].T @ F
...
def test_link():
    K = 100
    #jl1 = StickJointLink(1, 2, RevoluteJoint(), 30*np.pi/180)
    jl1 = StickJointLink(1, 2, RevoluteJoint(), 30*np.pi/180)
    #jl2 = StickJointLink(1, 1, RevoluteJoint(), 30*np.pi/180)
    jl2 = StickSpringJointLink(1, 1, K, RevoluteJoint(), 30*np.pi/180)
    I1 = jl1.Ic[0][0]
    I2 = jl2.Ic[0][0]
    ...
    while True:
        ...
        viewer.text([f"{k:.03f} + {u:.03f} = {k+u:.03f}",
            f"{q}"])
